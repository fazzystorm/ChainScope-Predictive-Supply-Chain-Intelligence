...
def main():
    file_path = r"c:\Users\hp\.gemini\antigravity\scratch\chainscope\index.html"
    with open(file_path, "r", encoding="utf-8") as f:
        html = f.read()

    # Change 1: Leaflet -> MapLibre GL
    html = re.sub(
        r'<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.css" />\s*<script src="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.js"></script>',
    ...
    html = re.sub(r'        \.sidebar \{\s+background: var\(--s1\);\s+border-left: 1px solid var\(--b1\);\s+display: flex;\s+flex-direction: column;\s+overflow: hidden\s+\}', sidebar_css, html)

    workspace_css = """        .workspace {
            flex: 1;
            display: flex;
            overflow: hidden
        }"""
    html = re.sub(r'        \.workspace \{\s+flex: 1;\s+display: grid;\s+grid-template-columns: 1fr var\(--sb-w\);\s+overflow: hidden\s+\}', workspace_css, html)

    map_wrap_css = """        .map-wrap {
            flex: 1;
            position: relative;
            overflow: hidden
        }"""
    html = re.sub(r'        \.map-wrap \{\s+position: relative;\s+overflow: hidden\s+\}', map_wrap_css, html)

    html = html.replace('<div id="map"></div>', '<button class="sidebar-toggle" id="sb-toggle">❯</button>\n            <div id="map"></div>')

    # Now for the JS block.
    # The existing JS has map initialization and Leaflet logic. We need to rewrite `renderMap`, map initialization, etc.
    # Let's extract everything inside `<script>`
    
    start_str = "        // ═══ COMPANY DATA ═══"
    end_str = "        // ═══ EVENT BINDINGS ═══"
    company_data_match = html[html.find(start_str):html.find(end_str)].strip()
    
    # We will build the new script content
    new_script = """    <script>
        // ═══ COMPANY DATA ═══
"""
    new_script += company_data_match.replace("        // ═══ COMPANY DATA ═══", "").strip()
    ...
    <div class="section" id="sec-heat"><div class="s-head" onclick="toggleSection('sec-heat')"><div class="s-icon" style="background:rgba(192,132,252,.1)">🔥</div><div class="s-title">Industry Heatmap</div><div class="s-meta">${d.heatmap.length} regions</div><div class="s-arrow">▼</div></div><div class="s-body"><div class="s-content"><div class="heat-legend"><div class="heat-lbl">LOW</div><div class="heat-grad"></div><div class="heat-lbl">HIGH</div></div><div class="heat-rows">${heatHtml}</div></div></div></div>`;
            ...
            // Re-inject ships and news if available
        ...
        // Expose function for inline onclick of heatmap rows
        ...
        const layerState = { routes: true, heat: true, risk: true };
            ...
            if(window.map && window.map.isStyleLoaded()) {
                if(l === 'routes') {
                    if(layerState.routes) {
                        if(map.getSource('routes')) map.setLayoutProperty('routes-layer', 'visibility', 'visible');
                    } else {
                        if(map.getSource('routes')) map.setLayoutProperty('routes-layer', 'visibility', 'none');
                    }
                }
                if(l === 'heat') {
                    if(layerState.heat) {
                        if(map.getSource('heat')) map.setLayoutProperty('heat-layer', 'visibility', 'visible');
                    } else {
                        if(map.getSource('heat')) map.setLayoutProperty('heat-layer', 'visibility', 'none');
                    }
                }
                // Toggle markers visually for risk layer
                if(l === 'risk') {
        ...
        function renderMapFeatures(d) {
            ...
            d.heatCircles.forEach(h => {
                // approximate circle with a 32-sided polygon or use Maplibre circle layer
                heatFeatures.push({
                    "type": "Feature",
                  ...
                  .then(function(data){
                    ...
                    ships.forEach(function(s){
                      ...
                      new maplibregl.Marker({element:el})
                        .setLngLat([s.lon, s.lat]) // Note: maplibre uses lon, lat
    ...
    # Strip from the end_str onward to remove the old <script> content and closing tags
    end_index = html.find("    <script>")
    if end_index != -1:
        html = html[:end_index] + new_script + "\n</html>"

    with open("/tmp/chainscope.html", "w", encoding="utf-8") as f:
        f.write(html)
    
    # Finally, overwrite the actual file for the user
